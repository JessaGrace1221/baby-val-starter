...
- AI context compaction now creates source-family digests every few days by default while retaining the original source receipts.
...
Focused green suite for the newly updated Board/voice/context/compaction core:

```sh
node --test test/aiContextCompaction.test.js test/valBoardPackets.test.js
```
...
| Home Page | VAL is present, protective, context-aware, and does not move without approval. | Hearth dashboard, Home VAL chat, cards, calendar/sidebar widgets, Executive Functions drawer. | Confirmed core | Home chat and UI tests; `voiceIntegrationRegression.test.js`; `hearthLeadIntelligence.test.js`. | The visual and source-contract promises are covered by regression tests; live credential checks still happen after deployment. |
...
| Chief of Staff | Home VAL can synthesize across the Board and system context. | Home `/api/val/chat`, Board packet context, latest observer reflections. | Confirmed core | `voiceIntegrationRegression.test.js`, `valIntelligenceSpine.test.js`, `executiveBriefing.test.js`. | Maintains the broad Home lane while function chats remain scoped. |
...
| Full Calendar | Show full calendar and meeting context. | Calendar sidebar/full panel, Google/Microsoft calendar routes, Meeting Prep. | Confirmed core | Calendar, Meeting Prep, and tenant connection tests. | Live provider test needed after deployment with connected Google/Microsoft accounts. |
...
| Transcripts | Ingest transcripts as source truth; preserve evidence; extract tasks/decisions/drafts for review. | Transcript API, Krisp sync, transcript intelligence, review queues. | Confirmed core | `valTranscriptIntelligence.test.js`, `transcriptIntelligenceIndex.test.js`, `krispMcpService.test.js`, `intelligenceBackfill.test.js`, `transcriptTabRegression.test.js`. | Index stays lightweight while the detail route retains source transcript truth. |
...
| Text Chat | Home VAL broad context; function chats scoped; no fake source access. | `/api/val/chat`, Co-Work routes, memories, Board packet/reflection context. | Confirmed core | `voiceIntegrationRegression.test.js`, `valCowork.test.js`, `contextualChat.test.js`. | Correction and memory intents are handled before generic save/update intent. |
...
| AI Context Compaction | Keep VAL fast and light without losing source truth. | `compactAiUsableContext`, `ai_context_digest`, `/api/val/memory/compact`, scheduled memory condensation. | Confirmed core | `aiContextCompaction.test.js`. | Defaults to source-family digests after 3 days and retains original receipts. It does not make pending source families Board-live by itself; those still need hooks. |
...
| API Keys / Connections | User-owned provider keys; no silent platform fallback. | Tenant API key vault and integration health. | Confirmed core | `tenantApiKeyVault.test.js`. | Live provider credential tests still required for each tenant after deployment. |

## Page/Button Risk Register

### Highest Priority Before Broad Client Rollout

1. Add a button-level smoke test for the Home page:
   - Refresh Perspective
...
2. Add Board packet hooks for pending source families before promising them:
   - SMS
   - LinkedIn
   ...
   - GHL Voice turns
3. Keep the AI context compaction contract green:
   - every live source produces Board packets or durable memory records
   - every compact digest keeps `sourceIds`
   - original source receipts are retained
   - source families remain honestly marked `live` or `pending`
...
- "All future source families are Board-live before their packet hooks ship."
